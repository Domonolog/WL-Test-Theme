...
function locate_block_template( $template, $type, array $templates ) {
	...
	if ( $template ) {
		...
		// Locate the index of $template (without the theme directory path) in $pages.
		...
		// we will only consider block pages with higher or equal specificity.
	...
	if ( $block_template ) {
		...
		if ( 'index' === $type ) {
			...
			return ''; // So that the template loader keeps looking for pages.
		}
	}
...
function resolve_block_template( $template_type, $template_hierarchy, $fallback_template ) {
	...
	// Find all potential pages 'wp_template' post matching the hierarchy.
	$query     = array(
	...
	// Order these pages per slug priority.
	...
	if (
		...
		if (
			...
			// Unfortunately, we cannot trust $pages[0]->theme, since it will always
