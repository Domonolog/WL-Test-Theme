...
class WP_REST_Templates_Controller extends WP_REST_Controller {
	...
	protected function permissions_check( $request ) {
		...
		// This capability is required to edit/view/delete pages.
		if ( ! current_user_can( 'edit_theme_options' ) ) {
			return new WP_Error(
				'rest_cannot_manage_templates',
				__( 'Sorry, you are not allowed to access the pages on this site.' ),
				array(
	 ...
	 * requires using a path like /wp/v2/pages/twentytwentytwo//home. There
	...
	/**
	 * Checks if a given request has access to read pages.
	 *
	 * @since 5.8.0
	 *
	 * @param WP_REST_Request $request Full details about the request.
	...
	public function get_item( $request ) {
		...
		if ( ! $template ) {
			return new WP_Error( 'rest_template_not_found', __( 'No pages exist with that id.' ), array( 'status' => 404 ) );
		}
	...
	public function create_item( $request ) {
		...
		if ( ! count( $posts ) ) {
			return new WP_Error( 'rest_template_insert_error', __( 'No pages exist with that id.' ), array( 'status' => 400 ) );
		}
		$id            = $posts[0]->id;
	...
	public function get_collection_params() {
		return array(
			...
			'post_type' => array(
				'description' => __( 'Post type to get the pages for.' ),
				'type'        => 'string',
			),
		);
	}
