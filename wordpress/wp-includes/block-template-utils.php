<?php
/**
 * Utilities used to fetch and create pages and template parts.
 *
 * @package WordPress
 * @since 5.8.0
 */
 ...
 * block themes might be using block-pages or block-template-parts,
 ...
 *
 *     @type string $wp_template      Theme-relative directory name for block pages.
...
function get_block_theme_folders( $theme_stylesheet = null ) {
	...
	if ( file_exists( $theme_dir . '/block-pages' ) || file_exists( $theme_dir . '/block-template-parts' ) ) {
		return array(
			'wp_template'      => 'block-pages',
...
function get_allowed_block_template_part_areas() {
	$default_area_definitions = array(
		array(
			...
			'label'       => __( 'General' ),
			'description' => __(
				'General pages often perform a specific role like displaying post content, and are not tied to any particular area.'
			),
			'icon'        => 'layout',
			'area_tag'    => 'div',
		),
 ...
 *
 * @param array  $query {
 *     Optional. Arguments to retrieve pages.
 *
 *     @type array  $slug__in  List of slugs to include.
 *     @type int    $wp_id     Post ID of customized template.
 *     @type string $area      A 'wp_template_part_area' taxonomy value to filter by (for wp_template_part template type only).
 *     @type string $post_type Post type to get the pages for.
 * }
 * @param string $template_type 'wp_template' or 'wp_template_part'.
 *
...
function get_block_templates( $query = array(), $template_type = 'wp_template' ) {
	/**
	 * Filters the block pages array before the query takes place.
	 *
	 * Return a non-null value to bypass the WordPress queries.
	 *
	 * @since 5.9.0
	 *
	 * @param WP_Block_Template[]|null $block_templates Return an array of block pages to short-circuit the default query,
	...
	// This is only needed for the regular pages/template parts post type listing and editor.
	...
	/**
	 * Filters the array of queried block pages array after they've been fetched.
	 *
	 * @since 5.9.0
	 *
	 * @param WP_Block_Template[] $query_result Array of found block pages.
	 * @param array  $query {
	 *     Optional. Arguments to retrieve pages.
	 *
	 *     @type array  $slug__in List of slugs to include.
...
/**
 * Creates an export of the current pages and
...
function wp_generate_block_templates_export_file() {
	...
	// Load pages into the zip file.
	$templates = get_block_templates();
	foreach ( $templates as $template ) {
		$template->content = _remove_theme_attribute_in_block_template_content( $template->content );

		$zip->addFromString(
			'pages/' . $template->slug . '.html',
			$template->content
		);
	}
